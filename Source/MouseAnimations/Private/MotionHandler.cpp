...
MotionHandler::MotionHandler(TSharedPtr<ISequencer> Sequencer_,
...
MotionHandler::MotionHandler(const IKeyArea* KeyArea_, double Scale,
                             TSharedPtr<ISequencer> Sequencer_,
...
bool MotionHandler::IsValidMotionHandler() {
  ...
  bool IsBindingExistInSequencer =
      Sequencer->FindObjectsInCurrentSequence(Data.ObjectFGuid).Num() > 0;
...
void MotionHandler::SyncControlRigWithChannelValue(FFrameNumber InTime) {
  if (IsValid(MovieSceneControlRigParameterTrack)) {
    ...
    UE_LOG(LogTemp, Warning, TEXT("Trying to set value to control %s"),
           *Data.ControlSelection);
...
void MotionHandler::InitKeys() {
  ...
  if (Data.ChannelTypeName == "MovieSceneFloatChannel") {
    FloatChannel->GetData().UpdateOrAddKey(currentFrame,
                                           FMovieSceneFloatValue(value));
    FloatChannel->GetData().UpdateOrAddKey(nextFrame,
                                           FMovieSceneFloatValue(secondValue));
  } else if (Data.ChannelTypeName == "MovieSceneDoubleChannel") {
    DoubleChannel->GetData().UpdateOrAddKey(currentFrame,
                                            FMovieSceneDoubleValue(value));
    DoubleChannel->GetData().UpdateOrAddKey(
        nextFrame, FMovieSceneDoubleValue(secondValue));
  } else if (Data.ChannelTypeName == "MovieSceneIntegerChannel") {
