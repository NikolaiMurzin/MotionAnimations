...
MotionHandler::MotionHandler(ISequencer* Sequencer_,
                             UMovieSceneSequence* Sequence, FString FilePath) {
  ...
  OnCurrentIndexValueChanged.BindRaw(
      this, &MotionHandler::OnCurrentIndexValueChangedRaw);
  ...
  ChannelHandle =
      channelProxy.MakeHandle(FName(Data.ChannelTypeName), Data.ChannelIndex);
...
MotionHandler::MotionHandler(const IKeyArea* KeyArea_, double Scale,
                             ISequencer* Sequencer_,
                             UMovieSceneSequence* Sequence_,
                             UMovieSceneTrack* MovieSceneTrack_,
                             FGuid ObjectFGuid_, Mode Mode_) {
  Sequencer = Sequencer_;
  KeyArea = KeyArea_;
  PreviousValue = 0;
  IsFirstUpdate = true;
  ...
  FString ChannelTypeName =
      KeyArea->GetChannel().GetChannelTypeName().ToString();
  ...
  Data = FMotionHandlerData(
      Scale, ObjectFGuid_, TrackName_, RowIndex, ChannelTypeName, ChannelIndex,
      Mode_, Sequence_->GetDisplayName().ToString(), DataCustomName,
      ChannelHandle.GetMetaData()->DisplayText, KeyArea->GetName().ToString(),
      MovieSceneTrackName);
  SetControlRigTrack(MovieSceneTrack);
  SetMaterialTrack(MovieSceneTrack);
  SetNiagaraTrack(MovieSceneTrack);
...
bool MotionHandler::IsValidMotionHandler() {
  if (Sequencer == nullptr || MovieScene == nullptr ||
      MovieSceneTrack == nullptr) {
    ValidMotionHandler = false;
    return false;
  }
  bool ChannelNullPtr = (FloatChannel == nullptr && DoubleChannel == nullptr &&
                         BoolChannel == nullptr && IntegerChannel == nullptr);
...
void MotionHandler::SetControlRigTrack(UMovieSceneTrack* MovieSceneTrack_) {
  MovieSceneControlRigParameterTrack =
      Cast<UMovieSceneControlRigParameterTrack>(MovieSceneTrack_);

  if (IsValid(MovieSceneControlRigParameterTrack)) {
    TArray<FName> currentControlSelectionArr =
        MovieSceneControlRigParameterTrack->GetControlRig()
...
void MotionHandler::SetMaterialTrack(UMovieSceneTrack* MovieSceneTrack_) {
  UMovieSceneMaterialTrack* MovieSceneMaterialTrack_ =
      Cast<UMovieSceneMaterialTrack>(MovieSceneTrack_);
  if (MovieSceneMaterialTrack_ != nullptr &&
      IsValid(MovieSceneMaterialTrack_)) {
...
void MotionHandler::SetNiagaraTrack(UMovieSceneTrack* MovieSceneTrack_) {
  UMovieSceneParticleParameterTrack* MovieSceneParticleTrack_ =
      Cast<UMovieSceneParticleParameterTrack>(MovieSceneTrack_);
  UMovieSceneNiagaraTrack* MovieSceneNiagaraTrack_ =
      Cast<UMovieSceneNiagaraTrack>(MovieSceneTrack_);
  UMovieSceneNiagaraParameterTrack* MovieSceneNiagaraParameterTrack_ =
      Cast<UMovieSceneNiagaraParameterTrack>(MovieSceneTrack_);
  ...
  if (IsValid(MovieSceneNiagaraTrack_)) {
    ...
    if (IsValid(MovieSceneNiagaraParameterTrack_)) {
      UE_LOG(LogTemp, Warning,
             TEXT("It's Valid MovieSceneNiagaraParameterTrack_"));
      ...
      UMovieSceneNiagaraFloatParameterTrack* MovieSceneNiagaraFloatTrack_ =
          Cast<UMovieSceneNiagaraFloatParameterTrack>(
              MovieSceneNiagaraParameterTrack_);
      ...
      UMovieSceneNiagaraVectorParameterTrack* MovieSceneNiagaraVectorTrack_ =
          Cast<UMovieSceneNiagaraVectorParameterTrack>(
              MovieSceneNiagaraParameterTrack_);
      ...
      if (NiagaraComponent != nullptr) {
        ...
        if (MovieSceneNiagaraFloatTrack != nullptr) {
          ...
          FString VectorString =
              NiagaraComponent->GetOverrideParameters().ToString();
...
void MotionHandler::SyncMaterialTrack(FFrameNumber InTime) {
  ...
  if (IsValid(MovieSceneMaterialTrack)) {
    ...
    UE_LOG(LogTemp, Warning, TEXT("Data Channel DisplayText is  %s"),
           *ChannelDisplayText);
    if (FloatChannel == nullptr) {
      return;
    }
    float value;
    FloatChannel->Evaluate(InTime, value);
    if (ChannelDisplayText.Equals("R") || ChannelDisplayText.Equals("G") ||
        ChannelDisplayText.Equals("B") || ChannelDisplayText.Equals("A")) {
      ...
      MovieSceneMaterialTrack->AddColorParameterKey(FName(*ParamName), InTime,
                                                    color);
    } else {
      MovieSceneMaterialTrack->AddScalarParameterKey(ParameterName, InTime,
                                                     value);
      UE_LOG(LogTemp, Warning,
             TEXT("Set value to Movie Scene Material Scalar Track!"));
    }
  }
}
...
void MotionHandler::SetKey(FFrameNumber InTime, FVector2D InputVector) {
  if (!ValidMotionHandler) {
    UE_LOG(LogTemp, Warning,
           TEXT("Can't set key, motion handler is not valid"));
    return;
  }
  double valueToSet = 0;
  ...
  if (Data.ChannelTypeName == "MovieSceneFloatChannel") {
    ...
    if (!IsFirstUpdate) {
      ...
      ChannelData.UpdateOrAddKey(InTime,
                                 FMovieSceneFloatValue((float)valueToSet));
      InTime--;
      float prevValue = 0;
...
void MotionHandler::SyncControlRigWithChannelValue(FFrameNumber InTime) {
  if (IsValid(MovieSceneControlRigParameterTrack)) {
    ...
    FRigControlElement* controlElement =
        controlRig->FindControl(FName(Data.ControlSelection));
    ...
    FRigControlValue controlValue =
        controlRig->GetControlValue(FName(Data.ControlSelection));
    ...
    if (controlType == ERigControlType::Float) {
      controlRig->SetControlValue(FName(Data.ControlSelection), valueOfChannel,
                                  true, FRigControlModifiedContext(), true,
                                  true);
    } else if (controlType == ERigControlType::EulerTransform) {
      FEulerTransform eulerValue;
      eulerValue.FromFTransform(controlValue.GetAsTransform(
          ERigControlType::EulerTransform, ERigControlAxis::X));
      ...
      controlRig->SetControlValue(FName(Data.ControlSelection), eulerValue,
                                  true, FRigControlModifiedContext(), true,
                                  true);
    } else if (controlType == ERigControlType::TransformNoScale) {
      FTransformNoScale nValue = controlValue.GetAsTransform(
          ERigControlType::TransformNoScale, ERigControlAxis::X);
      ...
      FTransform eulerValue = controlValue.GetAsTransform(
          ERigControlType::Transform, ERigControlAxis::X);
      FVector location = eulerValue.GetLocation();
      UE::Math::TQuat<double> rotation = eulerValue.GetRotation();
      FVector scale = eulerValue.GetScale3D();
      FRotator rotator = rotation.Rotator();
      ...
      FVector minValue =
          GetVectorFromString(*controlValueMin.ToString<FVector2D>());
      FVector maxValue =
          GetVectorFromString(*controlValueMax.ToString<FVector2D>());
      ...
      controlRig->SetControlValue(FName(Data.ControlSelection), vec, true,
                                  FRigControlModifiedContext(), true, true);
    }
  }
}
...
FGuid MotionHandler::GetObjectFGuid() { return Data.ObjectFGuid; }
void MotionHandler::SetSelectedMode(Mode Mode_) { Data.SelectedMode = Mode_; }
Mode MotionHandler::GetSelectedMode() { return Data.SelectedMode; }
bool MotionHandler::SaveData() { return Data.Save(); }
bool MotionHandler::DeleteData() { return Data.Delete(); }
  ...
  if (MovieScene != nullptr) {
    ...
    if (Data.ChannelTypeName == "MovieSceneFloatChannel") {
      ...
      TArray<FMovieSceneFloatValue> Values =
          Data.KeyValues[Data.CurrentIndex].FloatValues;
      if (FloatChannel != nullptr) {
      ...
      TArray<FMovieSceneDoubleValue> Values =
          Data.KeyValues[Data.CurrentIndex].DoubleValues;
      if (DoubleChannel != nullptr) {
...
bool MotionHandler::operator==(MotionHandler& handler) {
  return (Data.GetName() == handler.Data.GetName() &&
          Data.ControlSelection == handler.Data.ControlSelection &&
          Data.ChannelDisplayText.ToString() ==
              handler.Data.ChannelDisplayText.ToString() &&
          Data.KeyAreaName == handler.Data.KeyAreaName &&
          Data.TrackDisplayName == handler.Data.TrackDisplayName);
}
void MotionHandler::OnScaleValueChangedRaw(double value) { Data.Scale = value; }
void MotionHandler::OnCurrentIndexValueChangedRaw(int32 value) {
  UE_LOG(LogTemp, Warning, TEXT("TRYING TO SET INDEX : %d"), value);
