		...
		SNew(SVerticalBox)
			...
			[
				...
				[
					...
					.OnClicked(FOnClicked::CreateSP(this, &SMouseAnimationsMenu::OnTestButtonClicked))
				]
			]
			+SVerticalBox::Slot()
			.AutoHeight()
			[
				...
				[
					SNew(SCheckBox)
					.OnCheckStateChanged(FOnCheckStateChanged::CreateSP(this, &SMouseAnimationsMenu::OnTestCheckboxStateChanged ))
				]
			]
	];
}
...
void SMouseAnimationsMenu::Tick(const FGeometry & AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	std::cout << "print cursor X";
	std::cout << cursorPos.X << std::endl;
	std::cout << "print cursor Y";
